...
import numpy as np
import pandas as pd
import requests, sqlite3, time, json, zlib, re
import os
from src import parse, dataframes
...
def create_tables(db_file):
	...
	if not os.path.exists(dbpath):
		os.mkdir(dbpath)
		os.mkdir(os.path.join(dbpath, 'backup'))
	...
	cur.executemany(sql, csv_content(os.path.join('input', 'ctycodes.csv'), 3))

	# Insert list of currencies into Currencies table
	sql = '''INSERT OR IGNORE INTO Currencies (currency, currency_code)
			 VALUES (?, ?)'''
	cur.executemany(sql, csv_content(os.path.join('input', 'symbols.csv'), 2))
	...
	cur.executemany(sql, csv_content(os.path.join('input', 'ms_investment-types.csv'), 2))
...
def fetch(db_file):
	...
	while True:
		...
		try:
			msg = 'Qty. of records to be updated (default=1000):\n'
			user_input = input(msg).strip()
			stp = int(user_input) if user_input else 1000
		except KeyboardInterrupt:
			print('\nGoodbye!')
			exit()
		except Exception:
...
def fetch_api(url_info):
	...
	print_progress(url_id, num, ct)
		...
		print_('')
		print('\n\tError: No data found for URL ID {}'.format(url_id))
		return
